require( ['jquery','pmms', 'datagrids','ge-bootstrap','datepicker','highchart','highchart-3d','exporting'], function($,pmms) {//'jquery', 'datagrids'   'jquery', 'datatables','bootstrap','chosen'    ,'highchart','highchart-3d','exporting'
	...
	$("#btn-byYear").on("click",function(){
		$.ajax({
			url:'/PMMS/restful/chart/getSpNumByYear',
			type:'post',
			dataType:'json',
			cache:false,
			data:{
...
//				alert('success!!!');
				...
				chart = $('#maint-quantity').highcharts(options);
...
//						 "plugins":['G'],
	...
	function getChartData(){
		...
		//判断是按单条备件还是五大部门查询
		if(spId!=''){
			url = "/PMMS/restful/chart/getSpNumBySpId";
		}else{
			url = "/PMMS/restful/chart/getSpNumBydept";
		}
		$.ajax({
			url:url,
			type:'post',
...
//				alert('success!!!');
	...
	function buildTable(aoColumns,dataList){
		$('#maint-quantity-div').empty().append('<div class="input-append flR pull-right">'+
				...
				'</div><table id="maint-quantity-table" class="table table-bordered table-striped tablebrdr mrgzero width100p" data-table-name="maint-quantity-table"></table>');
		
		$('#maint-quantity-table').iidsBasicDataGrid( {//iidsBasicDataGrid  dataTable
			"bProcessing": true,
	        "bPaginate":true,
	        "bAutoWidth": false,
	        "bSort": true,

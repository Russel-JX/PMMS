...
require( ['jquery', 'pmms','datagrids','ge-bootstrap','datepicker','highchart','highchart-3d','exporting'], function($,pmms) {//'jquery', 'datagrids'   'jquery', 'datatables','bootstrap','chosen'    ,'highchart','highchart-3d','exporting'
	...
	var options = {
		...
		tooltip : {
			headerFormat : '<span style="font-size:15px">{point.key}维修时间</span><table>',
	...
	$("#btn-byYear").on("click",function(){
		$.ajax({
			url:'/PMMS/restful/chart/getMTTRForYears',
			type:'post',
			dataType:'json',
			cache:false,
			data:{
...
//				alert('success!!!');
				...
				chart = $('#chart-mttr').highcharts(options);
...
//						 "plugins":['G'],
	...
	function getChartData(){
		...
		//判断是按单条设备还是五大部门查询
		if(equipId!=''){
			url = "/PMMS/restful/chart/getMTTRForSingleEquip";
		}else{
			url = "/PMMS/restful/chart/getMTTRForDepts";
		}
		$.ajax({
			url:url,
			type:'post',
...
//				alert('success!!!');
	...
	function buildTable(aoColumns,dataList){
		$('#chart-mttr-div').empty().append('<div class="input-append flR pull-right">'+
				'<input type="text" class="input-medium search-query" data-filter-table="chart-mttr-table"><button class="btn btn-icon"><i class="icon-search"></i></button>'+
				'</div><table id="chart-mttr-table" class="table table-bordered table-striped tablebrdr mrgzero width100p" data-table-name="chart-mttr-table"></table>');
		
		$('#chart-mttr-table').iidsBasicDataGrid( {//iidsBasicDataGrid  dataTable
			"bProcessing": true,
	        "bPaginate":true,
	        "bAutoWidth": false,
	        "bSort": true,

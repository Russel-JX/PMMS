...
require( ['jquery','pmms', 'datagrids','ge-bootstrap','datepicker','highchart','highchart-3d','exporting'], function($,pmms) {//'jquery', 'datagrids'   'jquery', 'datatables','bootstrap','chosen'    ,'highchart','highchart-3d','exporting'
	...
	var options = {
		...
		tooltip: {
		    headerFormat: '<span style="font-size:15px">{point.key}维修时间（月）</span><table>',
	...
	$("#btn-byYear").on("click",function(){
		$.ajax({
			url:'/PMMS/restful/chart/getEquipMTBFByYr',
			type:'post',
			dataType:'json',
			cache:false,
			data:{
...
//				alert('success!!!');
				...
				chart = $('#maint-gap').highcharts(options);
...
//						 "plugins":['G'],
	...
	function getChartData(){
		...
		if(equipId!=''){
			...
			url = "/PMMS/restful/chart/getEquipMTBFByEqId";
		}else{
			url = "/PMMS/restful/chart/getSignleYrEquipMTBF";
		}
...
//				alert('success!!!');
	...
	function buildTable(aoColumns,dataList){
		$('#maint-gap-div').empty().append('<div class="input-append flR pull-right">'+
				'<input type="text" class="input-medium search-query" data-filter-table="maint-gap-table"><button class="btn btn-icon"><i class="icon-search"></i></button>'+
				'</div><table id="maint-gap-table" class="table table-bordered table-striped tablebrdr mrgzero width100p" data-table-name="maint-gap-table"></table>');
		
		$('#maint-gap-table').iidsBasicDataGrid( {//iidsBasicDataGrid  dataTable
			"bProcessing": true,
	        "bPaginate":true,
	        "bAutoWidth": false,
	        "bSort": true,
